...
def run_greedy_scalability_experiment():
    ...
    # NEW: Total deliveries covered (count) vs n
    plt.figure(figsize=(9,6))
    plt.plot(df['size'], df['deliveries_covered'], marker='o')
    plt.xlabel("Number of delivery points (n)")
    plt.ylabel("Deliveries covered (count)")
    plt.title("Greedy Single-Path Scalability: Deliveries Covered vs n")
    plt.grid(True, alpha=0.4)
    plt.tight_layout()
    plt.savefig("Greedy_Scalability_DeliveriesCovered_vs_n.png", dpi=220)
    plt.close()
    print("Saved: Greedy_Scalability_DeliveriesCovered_vs_n.png")
    # OPTIONAL: Bar chart — runtime vs n
    df2  = df.sort_values('size').copy()
    plt.figure(figsize=(10,6))
    bars = plt.bar(df2['size'].astype(str), df2['computation_time_s'])
    plt.xlabel("Number of delivery points (n)")
    plt.ylabel("Computation time (s)")
    plt.title("Greedy Scalability: Time vs n (Bar)")
    plt.grid(True, axis='y', alpha=0.3)
    for b in bars:
        h = b.get_height()
    plt.annotate(f"{h:.2f}", xy=(b.get_x()+b.get_width()/2, h),
                 xytext=(0, 4), textcoords="offset points",
                 ha='center', va='bottom', fontsize=9)
    plt.tight_layout()
    plt.savefig("Greedy_Scalability_Time_Bar.png", dpi=220)
    plt.close()
    print("Saved: Greedy_Scalability_Time_Bar.png")

    df2['peak_mem_mb'] = df2['peak_mem_bytes'] / (1024 * 1024)

    plt.figure(figsize=(9,6))
    plt.plot(df2['size'], df2['peak_mem_mb'], marker='o')
    plt.xlabel("Number of delivery points (n)")
    plt.ylabel("Peak memory (MB)")
    plt.title("Greedy Single-Path Scalability: Peak Memory vs n")
    plt.grid(True, alpha=0.4)
    plt.tight_layout()
    plt.savefig("Greedy_Scalability_PeakMem_vs_n.png", dpi=220)
    plt.close()
    print("Saved: Greedy_Scalability_PeakMem_vs_n.png")
